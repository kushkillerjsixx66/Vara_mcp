...
def vara_run_scan(arguments: dict) -> dict:
    ...
    # Build config — tolerate both minimal (chatgpt) and extended (local) signatures
    cfg_kwargs = {
        "keywords": keywords,
    ...
    # Extended knobs (local derivative)
    for key in (
    ...
    try:
        ...
        # Fallback for older (chatgpt-branch style) signature
        cfg = VaraConfig(
            keywords=keywords,
            sweep_depth_hours=sweep,
            active_planes=planes,
    ...
    # Ensure output dir is under the data root the server knows about
...
def vara_get_scan(arguments: dict) -> dict:
    ...
    # also try exact full-id match inside files
...
def _render_fir_from_report(report: dict, arguments: dict) -> str:
    """Minimal Operator-Tier FIR renderer. Expand later to full FIR-011 template."""
